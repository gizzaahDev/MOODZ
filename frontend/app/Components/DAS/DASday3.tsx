...
const DASday3 = ({ navigation }: { navigation: any }) => {
  ...
  const updateActivityCount = async (activityName: string) => {
    try {
      const dayDoc = activitiesRef.doc('Day 3');
...
const updatedCount = currentActivities[activityIndex].count + 1;
  ...
  const handleMusicPress = async () => {
    await updateActivityCount('Music Player 3');
    router.replace("/Components/DAS/Music3");
  };
 
  const handleStoryPress = async () => {
    await updateActivityCount('Story Board');
    router.replace("/Components/DAS/DASStory");
  };

  const handleMemesPress = async () => {
    await updateActivityCount('Memes Corner');
