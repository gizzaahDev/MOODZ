...
import CommercializationDrawer from './SubComponents/CommercializationDrawer';
...
const Questionnaire = () => {
  ...
  const [showCommercializationDrawer, setShowCommercializationDrawer] = useState(false);
...
*Your results indicate a higher level of emotional distress. It's important to consult a qualified psychiatrist for professional support and care.`;
  ...
  const handleNextPress = async () => {
    if (prediction.toLowerCase() === 'high risk') {
      ...
      setShowCommercializationDrawer(true);
    }
  };
  ...
  const handleSelectPlan = (plan: string, price: number) => {
    setShowCommercializationDrawer(false);
    // Handle plan selection (e.g., log the selected plan and price)
  ...
  return (
    <>
            ...
            <View style={[styles.modalResultsContainer, { backgroundColor: theme.resultContaineBg }]}>
              ...
              <View style={[styles.ResultsContainer, { backgroundColor: theme.resultBg }]}>
                ...
                <View>
                  <View style={[styles.containerAnimeBackground, { backgroundColor: theme.resultTabCurve }]}>
                    ...
                    <Text style={[styles.Animeh1Predict, { color: theme.title }, prediction.toLowerCase() === 'high risk' && { color: theme.warningText },]}>{prediction}</Text>
                    <View style={styles.containerAnime}>
                      <LottieView
                        source={
                          prediction.toLowerCase() === 'high risk'
              ...
              <View style={styles.buttonContainer}>
                ...
                <TouchableOpacity
                  style={[
                    styles.buttonNext,
                    prediction.toLowerCase() === 'high risk' && styles.highButton,
                  ]}
                  onPress={handleNextPress}
                >
                  <Text style={styles.buttonTextNext}>
                    {prediction.toLowerCase() === 'high risk' ? 'Get Professional Help' : 'Next'}
                  </Text>
                </TouchableOpacity>
              </View>
      ...
      <CommercializationDrawer
        visible={showCommercializationDrawer}
        onClose={() => setShowCommercializationDrawer(false)}
