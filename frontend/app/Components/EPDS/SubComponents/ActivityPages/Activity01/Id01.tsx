...
const MeditationHome = () => {
  ...
  const fetchProgress = async () => {
    try {
      ...
      if (userDoc.exists) {
        ...
        const dates = Object.keys(meditationHistory);

        // Calculate total days meditated
        setTotalDays(dates.length);

        // Get today's session data (today's session if exists)
        const todayData = meditationHistory[today] || { streakDays: 0, playCount: 0, sessionDurations: [] };
  ...
  const recordMeditation = async () => {
    router.push('/Components/EPDS/SubComponents/ActivityPages/Activity01/Meditaion');
  };
