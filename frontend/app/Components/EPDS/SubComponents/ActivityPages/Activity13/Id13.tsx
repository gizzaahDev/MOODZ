    ...
    const fetchProgress = async () => {
        try {
            ...
            if (userDoc.exists) {
                ...
                // Get total days, play count and streak days
                const dates = Object.keys(dailyJournal);
                const totalDays = dates.length;
                const playCount = todayData.playCount || 0;
                const streakDays = todayData.streakDays || 0;
                ...
                dates.forEach((date) => {
                    markedDates[date] = { 
                        selected: true, 
                        selectedColor: '#016A70',
                        // Add duration as a custom property if available
                        // duration: dailyJournal[date].duration || 0 
                    };
                });
        ...
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            <View style={[styles.subMiniContainer]}>
                ...
                <View style={styles.buttonContainer}>
                    <TouchableOpacity
                        style={styles.button}
                        onPress={() => router.push('/Components/EPDS/SubComponents/ActivityPages/Activity13/CreativeArt')}>
