...
export default function Home() {
  ...
  const getQuestionnaireRoute = (category: string) => {
    switch (category) {
      case 'child':
        return '/Components/ChildDepression/Questionnaire';
      case 'marital':
  ...
  const handlePress = async (category: string) => {
    ...
    if (user) {
      ...
      const hasSubmitted = await hasUserSubmittedQuestionnaire(userId, category);
  ...
  const handleUserChoice = async (choice: 'questionnaire' | 'activities', category: string) => {
